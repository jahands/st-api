...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        model_path = "/app/model/decision_forest.pkl"
...
async def classify_text_in_image(image: Image.Image) -> tuple[str, float, int]:
    ...
    try:
        ...
        if not number_xpos_pairs:
            return "", 0.0, 0

        # Sort by x-position and extract the number string
        number_xpos_pairs.sort(key=lambda x: x[1])
        extracted_text = ''.join([pair[0] for pair in number_xpos_pairs])
        ...
        logger.error(f"Error in classify_text_in_image: {str(e)}")
        return "ERROR", 0.0, 0
